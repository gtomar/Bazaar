...
var chat_url = "http://erebor.lti.cs.cmu.edu:8015/chat/";
...
setInterval(function () 
{
  ...
  for (var id in sessions) 
  {
    ...
    //var exec = require('child_process').exec;
    exec('ps aux | grep java | wc -l',
        function (error, stdout, stderr) {
            if (error !== null) {
                winston.log('info', "exec error: " + error);
            }
            if (parseInt(stdout) > 200){
                winston.log('info', 'Heavy load  on server with number of agents : ' + stdout);
                server_busy = 1;
            }
            else
            {
    ...
    if(!server_busy && supplicants.length >= teamsize && now - lastJoin > GROUP_DELAY) //assemble a team
    {
    ...
    {
        member = session;
        winston.log('info', "advising student "+member.nick+" to try later");
        channel.appendMessage("System", "msg", member.nick+", There is a heavy load on our servers and all are agents are busy. Please try again later after some time.", member.nick);
        supplicants.splice(supplicants.indexOf(member),1);
        setTimeout(function(member)
        {
            return function()
